...
export default function ChatWidget() {
  ...
  return (
    <div className="fixed right-1 bottom-1 z-50">
      {/* Chat window — CSS transition instead of AnimatePresence */}
      <div
        className={`mb-4 flex h-120 w-85 flex-col overflow-hidden rounded-2xl border border-border bg-white shadow-2xl transition-all duration-200 sm:w-95 ${
          isOpen
            ? "visible scale-100 opacity-100"
            : "invisible pointer-events-none scale-95 opacity-0 translate-y-4"
        }`}
      >
        {/* Header */}
